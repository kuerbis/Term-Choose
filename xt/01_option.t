...
eval "use Test::FailWarnings";
plan skip_all => "Test::FailWarnings required to test options" if $@;
...
ok( $d = choose( $choices, {
    ...
    pad => 3, pad_one_row => 2, empty => '', prompt => '', undef => '', lf => [ 1 ], no_spacebar => [ 0 ] } ) || 1 );
...
ok( $d = choose( [ 'aaa' .. 'zzz' ], {
    ...
    mouse => 0, layout => 3, justify => 0, index => 0, hide_cursor => 1,  clear_screen => undef, beep  => 0 } ) || 1 );
