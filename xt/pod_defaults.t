...
my @long =
  qw( pad empty undef ll default max_height max_width lf keep no_spacebar mark );
my @simple =
  qw( justify layout order clear_screen page mouse beep hide_cursor index )
  ;    # prompt
my @all = ( @long, @simple );
...
is(
    scalar @all,
    scalar keys %option_default,
    'scalar @all == scalar keys %option_default'
);
is(
    scalar keys %pod_default,
    scalar keys %option_default,
...
for my $key ( sort keys %option_default ) {
    next if $key ~~ @skip;
    is( $option_default{$key}, $pod_default{$key},
        "option $key: default value in pod matches default value in code" );
}
