...
sub new {
    my $class = shift;
    my ($opt) = @_;
    croak "new: called with " . @_ . " arguments - 0 or 1 arguments expected"
      if @_ > 1;
    my $self = bless {}, $class;
    if ( defined $opt ) {
        croak "new: the (optional) argument must be a HASH reference"
          if ref $opt ne 'HASH';
...
sub __defaults {    #hae
    my ($self) = @_;
    my $prompt =
      defined $self->{wantarray} ? 'Your choice:' : 'Close with ENTER';
    return {
        prompt       => $prompt,
        info         => '',
        beep         => 0,
    ...
    for my $key ( keys %$opt ) {
        ...
        if ( $valid->{$key} eq 'ARRAY' ) {
              ...
              if ref $opt->{$key} ne 'ARRAY';
            {
                ...
                for ( @{ $opt->{$key} } ) {
                    /^[0-9]+\z/
                      or croak "$sub $key => $_ is an invalid array element";
                }
            }
            if ( $key eq 'lf' ) {
                croak "$sub $key => too many array elements."
                  if @{ $opt->{$key} } > 2;
            }
        }
...
sub __init_term {
    my ($self) = @_;
    $self->{mouse} =
      $self->{plugin}->__set_mode( $self->{mouse}, $self->{hide_cursor} );
}
...
sub choose {    #hae
    if ( ref $_[0] ne 'Term::Choose' ) {
        ...
        return __choose( bless( { plugin => $Plugin->new() }, 'Term::Choose' ),
            @_ );
    }
    my $self = shift;
...
sub __choose {
    ...
    croak "choose: called with "
      . @_
      . " arguments - 1 or 2 arguments expected"
      if @_ < 1 || @_ > 2;
    croak "choose: the first argument must be an ARRAY reference"
      if ref $orig_list_ref ne 'ARRAY';
    if ( defined $opt ) {
        croak "choose: the (optional) second argument must be a HASH reference"
          if ref $opt ne 'HASH';
        $self->__validate_and_add_options($opt);
    }
    if ( !@$orig_list_ref ) {
  ...
  GET_KEY: while (1) {
        ...
        if ( $key == VK_INSERT ) {
            $page_step = $fast_page
              if $self->{p_begin} - $fast_page * $self->{avail_height} >= 0;
            $key = VK_PAGE_UP;
        }
        elsif ( $key == VK_DELETE ) {
            $page_step = $fast_page
              if $self->{p_end} + $fast_page * $self->{avail_height} <=
              $#{ $self->{rc2idx} };
            $key = VK_PAGE_DOWN;
        }
        if (   $saved_pos
        ...
        if ( $key == VK_DOWN || $key == KEY_j ) {
            ...
            if ( $self->{pos}[COL] == 0 && $self->{pos}[ROW] == 0 ) {
                ...
                if ( $self->{pos}[COL] > 0 ) {
                    ...
                    if ( $self->{pos}[ROW] >= $self->{p_begin} ) {
                        ...
                        $self->{p_end} = $self->{p_begin} - 1;
                        $self->{p_begin} =
                          $self->{p_begin} - $self->{avail_height};
                        $self->{p_begin} = 0 if $self->{p_begin} < 0;
                        $self->{pos}[COL] =
                          $#{ $self->{rc2idx}[ $self->{pos}[ROW] ] };
                        $self->__wr_screen();
            ...
            if ( $self->{p_begin} <= 0 ) {
                ...
                $self->{p_begin} =
                  $self->{avail_height} *
                  (
                    int( $self->{pos}[ROW] / $self->{avail_height} ) -
                      $page_step );
            ...
            if ( $self->{p_end} >= $#{ $self->{rc2idx} } ) {
                ...
                {
                    $saved_pos = [ $self->{pos}[ROW] - $backup_p_begin,
                        $self->{pos}[COL] ];
                    ...
                    $#{ $self->{rc2idx}[ $self->{pos}[ROW] ] } )
                {
                    ...
                    if ( $self->{p_begin} == $#{ $self->{rc2idx} } ) {
                        $self->{p_begin} =
                          $self->{p_begin} - $self->{avail_height};
                        $self->{p_end} =
                          $self->{p_begin} + $self->{avail_height} - 1;
                    }
                    else {
                        $self->{p_end} = $#{ $self->{rc2idx} };
                    }
            ...
            if ( $self->{wantarray} ) {
                    ...
                    || defined $self->{meta_items} )
                {
                    ...
                    {
                        if ( $self->{rc2idx}[ $self->{pos}[ROW] ]
                            [ $self->{pos}[COL] ] == $no_spacebar )
                        {
                            ++$locked;
                            last;
                        }
...
sub __marked_idx2rc {
    ...
    if ( $self->{order} == 0 ) {
        ...
        my $end_last_full_col =
          $rows_per_col * ( $self->{rest} || $cols_per_row );
        for my $idx (@$list_of_indexes) {
            next if $idx > $last_list_idx;
...
sub __write_first_screen {
    ...
    ( $self->{term_width}, $self->{term_height} ) =
      $self->{plugin}->__get_term_size();
    ( $self->{avail_width}, $self->{avail_height} ) =
      ( $self->{term_width}, $self->{term_height} );
...
#                 reaches the right edge of the terminal on a non-MSWin32-OS
    ...
    if ( $self->{mouse} == 2 ) {
        $self->{avail_width} = MAX_COL_MOUSE_1003
          if $self->{avail_width} > MAX_COL_MOUSE_1003;
        $self->{avail_height} = MAX_ROW_MOUSE_1003
          if $self->{avail_height} > MAX_ROW_MOUSE_1003;
    }
    ...
    $self->{p_begin}          = 0;
    $self->{p_end} =
        $self->{avail_height_idx} > $#{ $self->{rc2idx} }
      ? $#{ $self->{rc2idx} }
      : $self->{avail_height_idx};
    $self->{i_row}  = 0;
    $self->{i_col}  = 0;
    $self->{pos}    = [ 0, 0 ];
    $self->{marked} = [];
...
sub __prepare_promptline {
    ...
    $self->{prompt_copy} =
      line_fold( $prompt, $self->{avail_width}, ' ' x $init, ' ' x $subseq );
    $self->{prompt_copy} .= "\n\r";
...
sub __size_and_layout {
    ...
    if ( $layout == 0 || $layout == 1 ) {
        for my $idx ( 0 .. $#{ $self->{list} } ) {
            ...
            $all_in_first_row .= ' ' x $self->{pad}
              if $idx < $#{ $self->{list} };
            if ( $self->__print_columns($all_in_first_row) >
                $self->{avail_width} )
            {
                $all_in_first_row = '';
                last;
    ...
    if ($all_in_first_row) {
        ...
        if ( $layout == 1 || $layout == 2 ) {
            ...
            if ( $tmc < $tmp_avail_width ) {
                $tmc = int( $tmc + ( ( $tmp_avail_width - $tmc ) / 1.5 ) )
                  if $layout == 1;
                $tmc = int( $tmc + ( ( $tmp_avail_width - $tmc ) / 4 ) )
                  if $layout == 2;
                $tmp_avail_width = $tmc;
            }
        }
        ...
        if ( $self->{order} == 1 ) {
            my $rows =
              int( ( @{ $self->{list} } - 1 + $cols_per_row ) / $cols_per_row );
            my @rearranged_idx;
            my $begin = 0;
            my $end   = $rows - 1;
...
sub __set_default_cell {
  ...
  LOOP: for my $i ( 0 .. $#{ $self->{rc2idx} } ) {
    ...
    $self->{p_begin} =
      $self->{avail_height} * int( $self->{pos}[ROW] / $self->{avail_height} );
    $self->{p_end} = $self->{p_begin} + $self->{avail_height} - 1;
    $self->{p_end} = $#{ $self->{rc2idx} }
      if $self->{p_end} > $#{ $self->{rc2idx} };
}
...
sub __prepare_page_number {
    ...
    #my $total_pp = 0;
    if (
        @{ $self->{rc2idx} } / ( $self->{avail_height} + $self->{pp_row} ) > 1 )
    {    # changelog
        my $total_pp = int( $#{ $self->{rc2idx} } / $self->{avail_height} ) + 1;
        my $total_pp_w = length $total_pp;
        $self->{footer_fmt} =
          '--- Page %0' . $total_pp_w . 'd/' . $total_pp . ' ---';
        if (
            length( sprintf $self->{footer_fmt}, $total_pp ) >
            $self->{avail_width} )
        {
            ...
            if (
                ...
                $self->{avail_width} )
            {
                $total_pp_w = $self->{avail_width}
                  if $total_pp_w > $self->{avail_width};
                $self->{footer_fmt} =
                  '%0' . $total_pp_w . '.' . $total_pp_w . 's';
            }
        }
    }
    else {
...
sub __wr_screen {
    ...
    if ( $self->{pp_row} ) {
        ...
        my $pp_line = sprintf $self->{footer_fmt},
          int( $self->{p_begin} / $self->{avail_height} ) + 1;
        print $pp_line;
...
sub __unicode_sprintf {
    ...
    if ( $str_length > $self->{avail_col_width} ) {
        if ( $self->{avail_col_width} > 3 ) {
            $unicode = $self->__unicode_trim( $self->{list}[$idx],
                $self->{avail_col_width} - 3 )
              . '...';
        }
        else {
            $unicode = $self->__unicode_trim( $self->{list}[$idx],
                $self->{avail_col_width} );
        }
    }
    elsif ( $str_length < $self->{avail_col_width} ) {
        if ( $self->{justify} == 0 ) {
            $unicode = $self->{list}[$idx]
              . " " x ( $self->{avail_col_width} - $str_length );
        }
        elsif ( $self->{justify} == 1 ) {
            $unicode = " " x ( $self->{avail_col_width} - $str_length )
              . $self->{list}[$idx];
        }
            ...
            my $half = int( $all / 2 );
            $unicode =
              " " x $half . $self->{list}[$idx] . " " x ( $all - $half );
        }
    }
    else {
        $unicode = $self->{list}[$idx];
...
sub __mouse_info_to_key {
  ...
  COL: for my $col ( 0 .. $#{ $self->{rc2idx}[$row] } ) {
        ...
        if ( $#{ $self->{rc2idx} } == 0 ) {
            ...
            $end_this_col =
              $end_last_col +
              $self->__print_columns( $self->{list}[$idx] ) +
              $self->{pad};
        }
        else {    #
...
C<Term::Choose> provides a functional interface (L</SUBROUTINES>) and
an object-oriented interface (L</METHODS>).
...
Options set with C<choose> overwrite options set with C<new>. Before
leaving C<choose> restores the overwritten options.
...
For more information how to use C<choose> and its return values see
L<USAGE AND RETURN VALUES>.

=head1 SUBROUTINES

=head2 choose

The function C<choose> allows the user to choose from a list. It takes
the same arguments as the method L</choose>.
...
See the L</OPTIONS> section for more details about the different
...
If C<choose> is called in a I<scalar context>, the user can choose an
item by using the L</Keys to move around> and confirming with
C<Return>.
...
If C<choose> is called in an I<list context>, the user can also mark an
item with the C<SpaceBar>.

C<choose> then returns - when C<Return> is pressed - the list of marked
items (including the highlighted item if the option
I<include_highlighted> is set to C<1>).

In I<list context> C<Ctrl-SpaceBar> (or C<Ctrl-@>) inverts the choices:
marked items are unmarked and unmarked items are marked.

=item *

If C<choose> is called in an I<void context>, the user can move around
but mark nothing; the output shown by C<choose> can be closed with
C<Return>.

Called in void context C<choose> returns nothing.

If the first argument refers to an empty array, C<choose> returns
nothing.
...
keystroke C<choose> rewrites the screen.

C<choose> returns C<undef> or an empty list in list context if the C<q>
key (or C<Ctrl-D>) is pressed.

With a I<mouse> mode enabled (and if supported by the terminal) the
item can be chosen with the left mouse key, in list context the right
mouse key can be used instead the C<SpaceBar> key.
...
the C<Arrow> keys (or the C<h,j,k,l> keys) to move up and down or to
move to the right and to the left,

=item *

the C<Tab> key (or C<Ctrl-I>) to move forward, the C<BackSpace> key (or
C<Ctrl-H> or C<Shift-Tab>) to move backward,

=item *

the C<PageUp> key (or C<Ctrl-B>) to go back one page, the C<PageDown>
key (or C<Ctrl-F>) to go forward one page,

=item *

the C<Insert> key to go back 25 pages, the C<Delete> key to go forward
25 pages,

=item *

the C<Home> key (or C<Ctrl-A>) to jump to the beginning of the list,
the C<End> key (or C<Ctrl-E>) to jump to the end of the list.
...
C<choose> returns the chosen elements as they were passed to the
...
If an element is not defined the value from the option I<undef> is
assigned to the element.

=item *

If an element holds an empty string the value from the option I<empty>
...
If the length of an element is greater than the width of the screen the
element is cut and at the end of the string are added three dots.

=back

The following should be without meaning if you comply with the
requirements.
...
Characters which match the Unicode character property C<Other> are
removed.

    $element =~ s/\p{C}//g;

C<ESC> characters are removed by this substitution so it is not
possible to color the output with ANSI escape sequences. For colored
output see L<Term::Choose_HAE>.

=item *

If an element is a reference it will be replaced with a string: the
reference type followed with the hexadecimal value of the reference
...
With the option I<default> it can be selected an element, which will be
highlighted as the default instead of the first element.

I<default> expects a zero indexed value, so e.g. to highlight the third
element the value would be I<2>.
...
Expects as its value a string. The string is printed above the prompt
string.
...
I<keep> prevents that all the terminal rows are used by the prompt
lines.

Setting I<keep> ensures that at least I<keep> terminal rows are
available for printing list rows.

If the terminal height is less than I<keep> I<keep> is set to the
terminal height.
...
If I<prompt> lines are folded the option I<lf> allows one to insert
spaces at beginning of the folded lines.

The option I<lf> expects a reference to an array with one or two
elements:

- the first element (C<INITIAL_TAB>) sets the number of spaces inserted
at beginning of paragraphs

- a second element (C<SUBSEQUENT_TAB>) sets the number of spaces
...
If I<ll> is set, then C<choose> doesn't calculate the length of the
longest element itself but uses the value passed with this option.

I<length> refers here to the number of print columns the element will
use on the terminal.

The length of undefined elements depends on the value of the option
I<undef>.

If the option I<ll> is set, only undefined values are replaced. The
replacements described in L</Modifications for the output> are not
applied. If elements contain unsupported characters the output might
break if the width (number of print columns) of the replacement
character does not correspond to the width of the replaced character -
for example when a unsupported non-spacing character is replaced by a
replacement character with a normal width.

If I<ll> is set to a value less than the length of the elements, the
output could break.

If the value of I<ll> is greater than the screen width, the elements
will be trimmed to fit into the screen.

If I<ll> is set, C<choose> returns (in list or scalar context) always
the indexes of the chosen items regardless of how I<index> is set.

If I<ll> is set and the window size has changed, choose returns
immediately C<-1>.
...
If the available height is less than I<max_height> then I<max_height>
is set to the available height.

Height in this context means print rows.

I<max_height> overwrites I<keep> if I<max_height> is set to a value
less than I<keep>.
...
If defined, sets the maximal output width to I<max_width> if the
terminal width is greater than I<max_width>.

To prevent the "auto-format" to use a width less than I<max_width> set
I<layout> to 0.
...
2 - mouse mode 1003 enabled; the output width is limited to 223
print-columns and the height to 223 rows (mouse mode 1003 doesn't work
above 223)
...
If a mouse mode is enabled layers for C<STDIN> are changed. Then before
leaving C<choose> as a cleanup C<STDIN> is marked as C<UTF-8> with
C<:encoding(UTF-8)>. This doesn't apply if the OS is MSWin32.

If the OS is MSWin32 there is no difference between the mouse modes 1,
3, and 4 - the all enable the mouse with the help of L<Win32::Console>.
...
1 - print the page number on the bottom of the screen if there is more
then one page. (default)

=head3 prompt

If I<prompt> is undefined a default prompt-string will be shown.

If the I<prompt> value is an empty string ("") no prompt-line will be
shown.
...
1 - C<choose> returns the items marked with the C<SpaceBar> plus the
highlighted item.

2 - C<choose> returns the items marked with the C<SpaceBar>. If no
items are marked with the C<SpaceBar>, the highlighted item is
returned.

=head3 mark

I<mark> expects as its value a reference to an array. The elements of
the array are list indexes. C<choose> preselects the list-elements
...
I<meta_items> expects as its value a reference to an array. The
elements of the array are list indexes. These elements can not be
marked with the C<SpaceBar> or with the right mouse key but if one of
these elements is the highlighted item it is added to the chosen items
...
I<no_spacebar> expects as its value a reference to an array. The
elements of the array are indexes of the list which should not be
markable with the C<SpaceBar> or with the right mouse key.

If an element is preselected with the option I<mark> and also marked as
not selectable with the option I<no_spacebar>, the user can not remove
the preselection of this element.

I<no_spacebar> elements greater than the last index of the list are
ignored.
...
If pressing a key results in an undefined value C<choose> warns with
C<EOT: $!> and returns I<undef> or an empty list in list context.
...
If L<Term::ReadKey> is available it is used C<ReadKey> to read the user
input and C<GetTerminalSize> to get the terminal size. Without
C<Term::ReadKey> C<getc> is used to read the input and C<stty size> to
get the terminal size.

If the OS is MSWin32 it is used L<Win32::Console> to read the user
input and to get the terminal size.
...
environment variable C<TC_AMBIGUOUS_WIDE> is set to a true value,
...
If the option "hide_cursor" is enabled, it is also required the support
...
If the OS is MSWin32 L<Win32::Console> is used, to emulate the behavior
...
Thanks to the L<Perl-Community.de|http://www.perl-community.de> and the
people form L<stackoverflow|http://stackoverflow.com> for the help.
