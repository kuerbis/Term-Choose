...
use Win32::Console
  qw( STD_INPUT_HANDLE ENABLE_MOUSE_INPUT ENABLE_PROCESSED_INPUT STD_OUTPUT_HANDLE
...
sub SHIFTED_MASK () {
    RIGHT_ALT_PRESSED | LEFT_ALT_PRESSED | RIGHT_CTRL_PRESSED |
...
sub __get_key_OS {
    ...
    if ( $event_type == 1 ) {
        my (
            $key_down,    $repeat_count, $v_key_code,
            $v_scan_code, $char,         $ctrl_key_state
        ) = @event;
...
sub __clear_line {
    ...
    $self->{output}
      ->FillAttr( $self->{fill_attr}, $width, 0, $self->{abs_cursor_y} );
}

sub __bold_underline {
    my ($self) = @_;
    $self->{output}->Attr(
        $self->{curr_attr} | FOREGROUND_INTENSITY | BACKGROUND_INTENSITY );
}
