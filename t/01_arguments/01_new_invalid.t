...
$exception =
  exception { my $new = Term::Choose->new( { hello => 1, world => 2 } ) };
ok( $exception =~ $regex,
    "\$new = Term::Choose->new( { hello => 1, world => 2 } ) => $exception" );
...
for my $opt ( sort keys %$invalid_values ) {
    for my $val ( @{ $invalid_values->{$opt} } ) {
        my $exception =
          exception { $new{ $n++ } = Term::Choose->new( { $opt => $val } ) };
        ok( $exception =~ $regex,
            "\$new = Term::Choose->new( { $opt => $val } ) => $exception" );
    }
}
